...
// 动画中滑动的方向
typedef NS_ENUM(NSInteger, CHSideslipAnimationSlideDirection) {
    CHSideslipAnimationSlideNone,
    CHSideslipAnimationSlideRight,
    CHSideslipAnimationSlideLeft
};

@interface CHSideslipViewController ()
{
    ...
    CHSideslipAnimationSlideDirection _animationSlideDirection;
}
@end

@implementation CHSideslipViewController
...
+ (instancetype)sideslipViewControllerWithLeftViewController:(UIViewController *)leftViewController mainViewController:(UIViewController *)mainViewController {
    CHSideslipViewController *sideslipViewController = [[CHSideslipViewController shareInstance] initWithLeftViewController:leftViewController mainViewController:mainViewController];
...
- (instancetype)initWithLeftViewController:(UIViewController *)leftViewController mainViewController:(UIViewController *)mainViewController {
    ...
    if (self) {
        ...
        // 默认设置
        _animationType = CHSideslipAnimationTypeZoom;
        _animationSlideScale = 0.78;
        _animationZoomScale = 0.78;
        _shadowEnabled = NO;
        _animationSlideDirection = CHSideslipAnimationSlideNone;
...
+ (instancetype)shareInstance {
    static CHSideslipViewController *sideslipViewController = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sideslipViewController = [[CHSideslipViewController alloc] init];
    });
    return sideslipViewController;
}
...
- (void)setShadowForMainView {
    ...
    if (_animationSlideDirection == CHSideslipAnimationSlideRight) {
        bounds.size = CGSizeMake(kAnimationZoomScale * kScreenWidth, kAnimationZoomScale * kScreenHeight);
    } else if (_animationSlideDirection == CHSideslipAnimationSlideLeft) {
...
- (void)rightSharpPan {
    CGRect rect;
    if (_animationType == CHSideslipAnimationTypeZoom) {
    ...
    _animationSlideDirection = CHSideslipAnimationSlideRight;
    [self setShadowForMainView];
...
- (void)leftSharpPan {
    ...
    _animationComplete = NO;
    _animationSlideDirection = CHSideslipAnimationSlideLeft;
...
//    [[NSNotificationCenter defaultCenter] postNotificationName:CHSideslipMainViewHadGoneBackNotification object:mainViewFrameValue];
    
    if ([self.delegate respondsToSelector:@selector(sideslipMainViewDidScroll:mainViewFrameValue:)]) {
        [self.delegate sideslipMainViewDidScroll:self mainViewFrameValue:mainViewFrameValue];
    }
...
//    [[NSNotificationCenter defaultCenter] postNotificationName:CHSideslipMainViewDidScrollNotification object:mainViewFrameValue];
